...
This exporter queries the [Twelve Data](https://twelvedata.com/) quote endpoint on scrape and serves the result as Prometheus metrics, which is what lets a Prometheus instance retain prices of stocks, ETFs and mutual funds.

- 💹 **Quote Surface**: Price, previous close, change, percent change and volume, from one upstream call per symbol
- 🔎 **Per-Scrape Symbols**: The symbol list travels in the scrape URL, so a new instrument is a Prometheus config change
- ⏱️ **Bounded Upstream Calls**: Each request is capped at ten seconds, so a stalled upstream cannot hold a scrape open
- 📐 **Indicator Examples**: RSI written as a recording rule, in [`prometheus.rules.sample.yml`](./prometheus.rules.sample.yml)

> [!IMPORTANT]
> The exporter holds one Twelve Data API key and refuses to start without it, so register at [Twelve Data](https://twelvedata.com/) and generate a key first — [Getting Started — Authentication](https://twelvedata.com/docs#authentication) covers how.

> [!NOTE]
> The plan sets the credit budget and the reachable markets, and the exporter spends one credit per symbol per scrape, so the symbol count and the scrape interval together decide whether a plan holds. See [Pricing](https://twelvedata.com/pricing), [Credits](https://support.twelvedata.com/en/articles/5615854-credits) and [Stock Exchanges](https://support.twelvedata.com/en/collections/2787973-stock-exchanges).
...
The image declares `10016/tcp` with `EXPOSE` and publishes nothing, so `-p` is what makes the exporter reachable, and `-e` forwards `TWELVEDATA_API_KEY` from the shell rather than baking it into the image.
...
Point a Prometheus job at `/price` and pass the symbols as URL parameters — see [Prometheus Configuration](#prometheus-configuration) for the job and the recording rules.
...
The API key is the one required setting, and `TWELVEDATA_API_KEY` is the way to supply it. The `--twelvedata.api-key` flag takes the same value on the command line, where every account on the host reads it out of `ps`, so prefer the environment variable. The exporter exits at start-up when neither carries a value.

## Endpoints

The exporter serves two endpoints:

- `/` — landing page, which prints the query format and two example URLs when reached at <http://localhost:10016/>
- `/price` — metrics endpoint, configurable via `--web.scrape-path`, which needs a `symbols` parameter

> [!IMPORTANT]
> The `symbols` parameter takes a comma-separated list and may repeat, and every occurrence is concatenated into one list. A request carrying none returns an empty body with 200 rather than an error, so Prometheus counts that scrape as successful while every series is absent. Alert on the absence of `twelvedata_price`, not on `up`.

## Metrics

Every quote series is a gauge, and one scrape publishes all five of them for each symbol it resolved:
...
> [!IMPORTANT]
> `twelvedata_price` is computed as `previous_close + change` rather than read from the quote's `close` field, so it agrees with the two series beside it at every instant. A numeric field that fails to parse becomes `0`, which no label distinguishes from a genuine zero. A symbol whose request fails is skipped entirely, so its five series are absent for that scrape rather than zero — that difference is the one an alert can act on.
...
The exporter reads the key from the environment and starts listening; no symbol is named until a scrape arrives.
...
Open <http://localhost:10016/> to read the query format and follow the example URLs it prints.

### Prometheus Configuration

#### Job Configuration Example

Add the job from [`prometheus.sample.yml`](./prometheus.sample.yml) to your Prometheus configuration. The exporter reads its symbols from `params.symbols`, so that list and the job's `scrape_interval` are what set the credit spend.

#### Recording Rules Configuration Example

Add the rules from [`prometheus.rules.sample.yml`](./prometheus.rules.sample.yml) to your configuration. They derive technical indicators from the gauge series once per evaluation, which costs less than recomputing them in every dashboard query.
