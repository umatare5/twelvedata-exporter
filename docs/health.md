...
- It increments once where a scrape enters the collector rather than once per upstream call.
- The request rate the account is billed for is therefore that rate multiplied by the symbol count, not the counter itself.

**`twelvedata_failed_queries_total`**

- It is declared and registered but never incremented, so it stays `0` whatever the upstream answers.
...
- The three series above are the whole of it.
- The process and Go runtime collectors are registered on a second registry no handler serves, so no `go_` or `process_` series reach a scrape.
